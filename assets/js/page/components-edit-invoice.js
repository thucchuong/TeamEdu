$(document).ready(function () {
    $.ajax({
        type: 'POST',
        url: base_url + 'invoices/get_invoice_by_id',
        data: "id=" + invoice_id + "&" + csrfName + "=" + csrfHash,
        dataType: "json",
        success: function (result) {
            ...
            if (result.project_id == 0) {
                ...
                $('#project_id').val(result.project_id).trigger('change');
            }
...
$(document).on('click', '.add-invoice-item', function (e) {
    ...
    if (title != '') {
        ...
        i++;
        html = '<div class="invoice-item py-1">' +
            '<div class="row">' +
            '<input type="hidden" id=invoice_item_' + i + '_id name="invoice_item_id[]">' +
            ...
            '<input type="number" class="form-control quantity" name="quantity[]" id="item_' + i + '_quantity" onchange="update_amount(' + i + ')" min="1" placeholder="Qty">' +
            ...
            '</div>' +
            '<div class="col-md-1 custom-col">' +
            '<a href="#" class="btn btn-icon btn-danger remove-invoice-item" data-count=' + i + '><i class="fas fa-trash"></i></a>' +
            '</div></div>' +
            '</div>';

        $('#invoice_items').append(html);
        $('#invoice_item_' + i + '_id').val("");
        $('#item_' + i).val(item_id);
...
$('#edit_invoice_form').on('submit', function (e) {
    ...
    if ($("#edit_invoice_form").validate().form()) {
        $.ajax({
            type: 'POST',
