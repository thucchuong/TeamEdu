...
class Chat_model extends CI_Model
{
    ...
    {
        $query = "SELECT count(id) as total FROM messages WHERE type='$type' AND is_read=1 AND workspace_id=$workspace_id AND from_id=$from_id AND to_id=$to_id";
        $queryResult = $this->db->query($query);
        $total = $queryResult->result_array();
        ...
        foreach ($messages as $message) {
            ...
            if ($type == 'person') {
                if ($to_id == $message['to_id']) {
                    ...
                    $product[$i]['picture'] = mb_substr($me_user[0]['first_name'], 0, 1) . '' . mb_substr($me_user[0]['last_name'], 0, 1);
                    ...
                    $product[$i]['picture'] = mb_substr($oppo_user[0]['first_name'], 0, 1) . '' . mb_substr($oppo_user[0]['last_name'], 0, 1);
    ...
    function load_chat($from_id, $to_id,  $workspace_id, $type = '', $offset = '', $limit = '', $sort = '', $order = '', $search = '')
    {
        ...
        foreach ($messages as $message) {
            ...
            $product['msg'][$i]['picture'] = (string)mb_substr($me_user[0]['first_name'], 0, 1) . '' . (string)mb_substr($me_user[0]['last_name'], 0, 1);
    ...
    function get_user_picture($user_id)
    {
        ...
        $messages =  $query->result_array();
        $picture = mb_substr($messages[0]['first_name'], 0, 1) . '' . mb_substr($messages[0]['last_name'], 0, 1);
        return $picture;
    }
